...
arnmatch is a zero-dependency Python library that parses AWS ARNs into structured data. ARN patterns are auto-generated by scraping AWS official documentation.
...
- `__init__.py` - Main module with `arnmatch(arn)` function and `ARNMatch` dataclass
- `arn_patterns.py` - Generated file containing compiled regex patterns indexed by service

The `arnmatch()` function splits the ARN, looks up patterns by service, and returns an `ARNMatch` with partition, service, region, account, resource_type, and captured groups. The `resource_id` and `resource_name` properties use heuristics (prefer groups ending in "Id" or "Name").
